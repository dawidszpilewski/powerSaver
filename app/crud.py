...
def get_unapproved_computers(db: Session) -> List[models.Computer]:
    return db.query(models.Computer).filter(models.Computer.approved == False).all()
...
def update_computer(db: Session, computer: models.Computer, data: schemas.ComputerUpdate) -> models.Computer:
...
def create_daily_exclusion(db: Session, data: schemas.DailyExclusionCreate) -> models.DailyExclusion:
...
def exists_daily_exclusion_for_today(db: Session, computer_id: int) -> bool:
    today = date.today()
    return db.query(models.DailyExclusion).filter(
        and_(
